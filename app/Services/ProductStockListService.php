...
class ProductStockListService
{
    ...
    public function productStockList($data)
    {
        ...
        $productList = $products->filter(function ($product) use ($fromDate, $toDate, $receivedSums, $issueSums, $floorRecieveSums) {
            $id = $product->id;
            $totalReceived = $receivedSums[$id] ?? 0;
            $totalIssue = $issueSums[$id] ?? 0;
            $floorRecieve = $floorRecieveSums[$id] ?? 0;

            if ($fromDate && $toDate) {
                return $totalReceived || $totalIssue || $floorRecieve;
            }
