...
<template>
  ...
  <div class="p-4 bg-[#ffffff]">
    ...
    <EasyDataTable :headers="headers" :items="items" alternating :rows-per-page="50" :serch-field="searchField" :search-value="searchItem">
        <template #item-action="{ id, product, total_requisition }">
            <button v-if="page.props.user.can['receive-requisition']" @click="showModal(id, product, total_requisition)" class="bg-green-500 text-white font-bold py-2 px-4 rounded">Received</button>
        </template>
    </EasyDataTable>
