...
<template>
    ...
    <EasyDataTable :headers="headers" :items="items" alternating :rows-per-page="50" :search-field="searchField" :search-value="searchItem">
        <template #item-role="{ roles }">
            <span class="m-1" v-for="role in roles" :key="role.id">{{ role.name }} </span>

        </template>
        <template #item-action="{ id }">
            <Link v-if="page.props.user.can['update-user']" :href="`/user-save-page?user_id=${id}`" class="bg-blue-500 text-white font-bold py-2 px-4 rounded">Edit</Link>
            <button v-if="page.props.user.can['delete-user']" @click="deleteUser(id)" class="bg-red-500 text-white font-bold py-2 px-4 rounded ml-1 cursor-pointer">Delete</button>
        </template>
    </EasyDataTable>
</template>
